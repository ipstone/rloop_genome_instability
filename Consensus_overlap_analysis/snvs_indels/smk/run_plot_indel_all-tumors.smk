...
# There is some trouble in generating Serena-Davies_ER_Biallelic indel counts, so exclude it for now 2021-06-12
...
all_results = expand("output/indel_compare_rloop_regions/{tumor}/", tumor=tumors)
...
rule gen_sigs:
    output:
        directory("output/indel_compare_rloop_regions/{tumor}/")

    run:
        shell("mkdir -p {output[0]}")
        shell("Rscript lib/analyze_indel_freq_diff_rloops.R {wildcards.tumor}")
