rloop_genome_instability

code acompanying Rloop genome instability paper

* Hightlights:

  R-loop_consensus_regions/output:
    The folder contains the consensus R-loop regions (bed files) calculated in the paper.
    (We used the score 200 in the paper, for completeness, score 100 bed region file is also included).
    

* Folder/code structure:
  - R-loop_consensus_regions:
        scripts and final output for the consensus regions 


* Consensus_overlap_analysis
│   ├── R-loop_TSS_TES_regions: Intersect Rloop regions with TSS/TES site regions.
│   ├── snvs_indels: Analysis SNVs and Indels mutations frequencies in Rloop regions.
│   └── structural_variants: Analysis for structural variants breakpoints frequencies in Rloop regions.
